...
with st.container():
    if analysis_type == "Factors Influencing Bike Rental Demand":
        ...
        with col3:
            ...
            corr_matrix = filtered_data[['temp', 'hum', 'windspeed', 'cnt']].corr()
        ...
        with col4:
            ...
            sns.scatterplot(x=filtered_data[selected_factor], y=filtered_data['cnt'], hue=filtered_data['season'], palette='deep')
        ...
        avg_rentals = filtered_data.groupby('season')[selected_factor].mean().reset_index()
        
        plt.figure(figsize=(8, 4))
        sns.barplot(x='season', y=selected_factor, data=avg_rentals, palette='Blues_d')
        ...
        season_group = filtered_data.groupby('season')[['casual', 'registered']].mean().reset_index()
        ...
        total_users = filtered_data.groupby('season')[['casual', 'registered']].sum().reset_index()

        plt.figure(figsize=(8, 4))
        sns.barplot(x='season', y='casual', data=total_users, label='Casual Users', color='skyblue')
        sns.barplot(x='season', y='registered', data=total_users, label='Registered Users', color='lightcoral')
